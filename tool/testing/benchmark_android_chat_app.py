...
def patch_prefs(
    ...
    *,
    ...
    replacements = {
        ...
        '<string name="flutter.mmproj_path"></string>',
...
def dump_ui_values(adb: Path, serial: str) -> list[str]:
    run([str(adb), "-s", serial, "shell", "uiautomator", "dump", "/sdcard/bench_ui.xml"])
    xml = run([str(adb), "-s", serial, "shell", "cat", "/sdcard/bench_ui.xml"], capture=True)
...
def extract_metrics(values: list[str]) -> dict[str, str]:
    metrics: dict[str, str] = {}
    for value in values:
        if value in {"CPU", "VULKAN"}:
            metrics["backend_runtime"] = value
        elif value.startswith("avg "):
            metrics["avg"] = value
        elif value.startswith("decode "):
            metrics["decode"] = value
        elif value.startswith("first "):
            metrics["first"] = value
        elif value.startswith("total "):
            metrics["total"] = value
        elif value.startswith("p_eval "):
            metrics["p_eval"] = value
        elif value.startswith("eval "):
            metrics["eval"] = value
        elif value.startswith("sample "):
            metrics["sample"] = value
        elif value.startswith("reuse "):
            metrics["reuse"] = value
    return metrics
...
def main() -> int:
    ...
    parser.add_argument("--load-timeout-sec", type=int, default=180)
    parser.add_argument("--infer-timeout-sec", type=int, default=120)
    ...
    try:
        ...
        time.sleep(1)
        run([str(adb), "-s", args.serial, "shell", "input", "text", args.prompt])
        time.sleep(1)
        ...
        while time.time() < infer_deadline:
            values = dump_ui_values(adb, args.serial)
            metrics = extract_metrics(values)
            if "total" in metrics and args.prompt in values:
                print(
                    json.dumps(
                        {
                            "ok": True,
        ...
        print(json.dumps({"ok": False, "stage": "infer_timeout"}, indent=2))
        return 4
    finally:
